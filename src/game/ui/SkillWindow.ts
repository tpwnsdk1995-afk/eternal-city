...
export class SkillWindow extends Window {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number) {
    super(scene, 'skills', x, y, 620, 640, '스킬 (K)');
    this.refresh();
  }
  ...
  refresh(): void {
    ...
    for (const cat of SKILL_CATEGORIES) {
      ...
      for (const def of SKILLS.filter((k) => k.category === cat)) {
        ...
        this.content.add(this.scene.add.rectangle(10, y - 3, this.w - 20, 44, 0xffffff, active ? 0.08 : 0.03).setOrigin(0, 0));
    ...
    if (!this.elia) this.label(14, this.h - 62, '습득/랭크업은 광진구청 지하주차장의 EL.IA 기술정보원에게서 할 수 있습니다.', '#6b7280', 11);
  }
}
