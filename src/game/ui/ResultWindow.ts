...
export class ResultWindow extends Window {
  ...
  refresh(): void {
    ...
    if (!r.success) this.label(this.w / 2, 62, r.reason === 'timeout' ? '제한 시간을 초과했습니다.' : r.reason === 'booth' ? '지켜야 할 부스가 파괴되었습니다.' : r.reason === 'retreat' ? '작전을 포기하고 귀환했습니다.' : '작전 중 사망했습니다.', theme.colors.muted, 12).setOrigin(0.5, 0);
