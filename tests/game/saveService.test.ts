...
describe('save/load', () => {
  ...
  it('migrates a v1 (M1) save through v2 to the current version with empty quests and keeps everything else', () => {
    ...
    expect(migrated!.buffs).toEqual({ active: [] });
    expect(migrated!.character.race).toBe('human');
    expect(migrated!.guild).toEqual({ name: null, contributed: 0, foundedAt: 0 });
    expect(migrated!.storage).toEqual({ items: [], nextUid: 1 });
    ...
    expect(migrateSave(v3)!.version).toBe(8);
    const { buffs: _b, ...v4 } = { ...v2, version: 4 as number };
    expect(migrateSave(v4)!.version).toBe(8);
    const { storage: _st, ...v7 } = { ...v2, version: 7 as number };
    expect(migrateSave(v7)!.storage).toEqual({ items: [], nextUid: 1 });
